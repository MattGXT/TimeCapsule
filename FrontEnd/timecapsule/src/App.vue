<template>
  <nav>
    ...
    <div class="end">
      <button class="btn-logout" @click="logout()" v-show="token !== null">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          <path
...
main {
  background-image: url("~@/assets/bg-empty.png"),url('~@/assets/cloud.png');
  background-color: #a1eafb;
  height: calc(100vh - 56px);
  background-repeat: no-repeat;
  background-size: cover;
  background-position: center,top right;
  animation: clouds_anim 30s linear infinite;
}

@keyframes clouds_anim{
  from {background-position: center,top right -990px;}
  to {background-position: center,top left -990px;}
}
